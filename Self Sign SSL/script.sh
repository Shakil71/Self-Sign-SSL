...
sudo mkdir -p {root-ca,sub-ca,server}
sudo mkdir -p root-ca/{private,certs,index,serial,pem,crl,csr}
sudo mkdir -p sub-ca/{private,certs,index,serial,pem,crl,csr}
sudo mkdir -p server/{private,certs,index,serial,pem,crl,csr}
sudo mkdir generated
sudo touch root-ca/index/index
sudo touch sub-ca/index/index
sudo cp root-ca.conf root-ca
sudo cp sub-ca.conf sub-ca
...
sudo cp root-ca/certs/ca.crt generated
sudo cp sub-ca/certs/sub-ca.crt generated
sudo cp server/certs/server.crt generated
sudo cp server/private/server.key generated
sudo cp server/certs/server.pfx generated
echo "${gr}\n ================ SUCCESSFULLY GATHERED NECESSARY FILES ================ \n${nc}"
